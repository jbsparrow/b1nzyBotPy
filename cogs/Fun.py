...
def randomhex():
    ...
    hexValues = list("123456789ABCDEF")
    # Create the hex code.
    hex = f"0x{''.join(random.choice(hexValues) for i in range(6))}"
    # Convert into usable hex and return
    hex = int(hex, 16)
    return hex
...
class Fun(commands.Cog):
    ...
    async def coinflip(self, ctx, side=None):
        ...
        if side in set(["heads", "tails", "h", "t"]):
        ...
        elif not side or side in ['none', 'flip', 'n/a', 'nill', 'null', 'nada']:
        ...
        # Error handling.
            ...
            await ctx.send('Error: Side must be either heads or tails.', delete_after=25)
        ...
        if user is None:
            async with aiohttp.ClientSession() as session:
                async with session.get('https://some-random-api.ml/animu/pat') as g:
                    js = await g.json()
                    embed = discord.Embed(colour=randomhex(), description='*Gives you a headpatt.*')
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get('https://some-random-api.ml/animu/pat') as g:
                    js = await g.json()
                    embed = discord.Embed(colour=randomhex(), description=f'*<@{message.author.id}> gives <@{message.mentions[0].id}> a headpat.*')
                    embed.set_image(url=f'{js["link"]}')
        ...
        if user is None:
            async with aiohttp.ClientSession() as session:
                async with session.get('https://some-random-api.ml/animu/hug') as g:
                    js = await g.json()
                    embed = discord.Embed(colour=randomhex(), description='*Hugs you.*')
                    embed.set_image(url=f'{js["link"]}')
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get('https://some-random-api.ml/animu/hug') as g:
                    js = await g.json()
                    embed = discord.Embed(colour=randomhex(), description=f'*<@{message.author.id}> hugs <@{message.mentions[0].id}>.*')
                    embed.set_image(url=f'{js["link"]}')
    ...
    async def rand(self, ctx, min_num: int, max_num: int):
    ...
    async def uwu(self, ctx, *, text):
        ...
        text = text.replace("the", "de")
        text = text.replace("you", "yuw")
        text = text.replace("thought", "fought")
        text = text.replace("them", "dem")
        text = text.replace("with", "wif")
        text = text.replace("their", "deiw")
        text = text.replace("they're", "dey'we")
        text = text.replace("theyre", "dey'we")
        text = text.replace("these", "dese")
        await ctx.send(f"{text} UwU")
        ...
        if query == 'random':
            ...
            for r in ran:
                while limit != 0:
                    ...
                    embed = discord.Embed(colour=randomhex(), title=r.word)
